...
const SubFlyerAdd = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    let data = {rotation, scale, is_flipped, is_lock, x, y, height, width, order_by, font_vertical_spacing, text, flyerId, image_url}
   
    axios.post('http://localhost:3001/create-subflyer', data).then((response) => {
    ...
    <div className='mt-40 text-center bg-gray-200 space-x-8' style={{ margin: 'auto', height: '600px', }}>
        ...
        <form onSubmit={handleSubmit} className='text-center' >
            <div className='col-lg-12  pt-7 pb-7'>
                ...
                <label className='mt-5 ml-6'> flyerId:    </label>
                    <input value={flyerId} onChange={e => SetFlyerId(e.target.value)} type="text" name="name" className='outline  ml-3 ' />
